...
Private API routes should depend on `aurea_api.auth.get_authenticated_user`. It accepts only Bearer tokens verified against the configured Supabase project's asymmetric JWKS, exact issuer, configured audience, expiration, and UUID subject. The issuer and JWKS URL are derived from `AUREA_SUPABASE_URL`; private routes must use the returned `AuthenticatedUser` rather than client-supplied owner identity.
