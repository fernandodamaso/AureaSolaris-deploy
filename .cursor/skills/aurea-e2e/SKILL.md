...
1. Read [e2e/catalog/README.md](../../../e2e/catalog/README.md) and choose the requested ids. If no feature was named, run all CI ids plus agent playbooks except `hermes-live-provider`.
...
5. Apply the feature filter to Playwright itself. Set `AUREA_E2E_URL` to the sandbox URL and pass only the mapped spec files to `npx playwright test <spec files> --config=e2e/playwright.config.ts`. Multiple requested areas may pass multiple files.
6. Run in-scope non-Hermes visual playbooks in the reset test-user Chrome profile. For mocked/default Hermes, do **not** send a message from standalone Chrome: run `e2e/specs/hermes.spec.ts` with `--headed` so `installHermesMocks(page)` remains active. Use standalone/live Hermes only when the person explicitly requested a live provider in this session.
...
|---|---|
| boot | `e2e/specs/boot.spec.ts` |
| shell | `e2e/specs/shell.spec.ts` |
| astrologia, mandala | `e2e/specs/astrologia.spec.ts` |
| caderno | `e2e/specs/caderno.spec.ts` |
| diario, histórico, notas | `e2e/specs/diario.spec.ts` |
| agenda | `e2e/specs/agenda.spec.ts` |
| saúde | `e2e/specs/saude.spec.ts` |
| memórias | `e2e/specs/memorias.spec.ts` |
| hermes, study-loop | `e2e/specs/hermes.spec.ts` |
