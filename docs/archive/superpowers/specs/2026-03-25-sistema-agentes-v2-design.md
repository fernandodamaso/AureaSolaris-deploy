...
> **Objetivo:** Memória de longo prazo, sensores proativos  
> **Dependências:** Etapas 1-3

> ⚠️ **FEATURES POSTERGADAS:** Os sub-itens abaixo foram **adiados para v2.1** conforme decisão de simplificação YAGNI. Eles permanecem documentados para referência futura, mas não fazem parte do escopo atual.
> - **Autoaprendizado (4.3)** — Feedback loop dinâmico, detecção de padrões, ajuste de system prompt
> - **Coordenação Autônoma (4.4)** — Comunicação direta entre agentes sem intervenção do Dr. Strange
...
**Arquitetura:** Motor de sensores em Rust (`sensors.rs`) com `std::thread::spawn` + eventos Tauri **(simplificado: polling no frontend em v2)**

### 4.3 Autoaprendizado [POSTERGADO]

> 🚧 **Status: Adiado para v2.1** — Sistema de prompts fixos por enquanto
...
### 4.4 Coordenação Autônoma [POSTERGADO]

> 🚧 **Status: Adiado para v2.1** — Apenas delegação manual via Dr. Strange nesta versão
...
> **Nota:** Model names devem ser configurados como aliases/constants em `src/config/modelRouter.ts` ao invés de values hardcoded, permitindo mudanças sem alterações no código.

```typescript
// Model aliases (config/constants)
const MODEL_ALIASES = {
  FREE: 'llama3.2',           // Ollama local
  CHEAP: 'openai/gpt-4o-mini',
  SMART: 'anthropic/claude-3.5-sonnet',
  ASTRO: 'google/gemini-2.0-flash'
} as const;

const ROUTES = [
  { condition: input simples → MODEL_ALIASES.FREE },
  { condition: análise financeira → MODEL_ALIASES.CHEAP },
  { condition: interpretação astrológica → MODEL_ALIASES.SMART },
  { condition: código/debug → MODEL_ALIASES.CHEAP },
  { condition: fallback → MODEL_ALIASES.CHEAP },
];
...
| Regra | Implementação |
|-------|---------------|
| Rate limiting | 1 msg/segundo por agente (2 msg/segundo para Stark exec commands) |
...
| **SQL injection protection** | **Nota:** Este stack (Tauri + JSON file storage) não utiliza SQL em produção. Caso futura migração para SQLite seja considerada, implementar prepared statements via `sqlx`/`rusqlite` com parâmetros parametrizados, e validar/escapar todas as entradas. |
...
| `src-tauri/src/sensors.rs` | Motor de sensores (simplificado: polling no frontend) |
| `src-tauri/src/patterns.rs` | Detecção de padrões **[POSTERGADO para v2.1]** |
| `src-tauri/src/coordination.rs` | Comunicação inter-agente **[POSTERGADO para v2.1]** |
