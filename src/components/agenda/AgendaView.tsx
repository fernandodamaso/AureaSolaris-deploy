    ...
    <div className="space-y-12 pb-32 animate-in fade-in max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
         <div className="space-y-8">
            ...
            <Card title={`Compromissos - ${selectedDay.toLocaleDateString('pt-BR')}`} icon={<Clock size={18}/>}>
               <div className="space-y-4 mt-4">
                  ...
                  <button className="w-full py-4 mt-2 border border-dashed border-gold/20 text-gold text-[10px] font-bold uppercase rounded-xl hover:bg-[#FCF9F1] transition-all">+ Agendar no Google Calendar</button>
               </div>
            </Card>
         </div>
         ...
         <div className="space-y-8">
            ...
            <Card title="Agenda Previsional (30 dias)" icon={<CalendarDays size={18}/>}>
        ...
        <div className="fixed inset-0 z-[700] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
           <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl border border-gold/20">
              <div className="flex justify-between items-center mb-6">
                 <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-gold">Nova Tarefa Todoist</h4>
                 <X size={18} className="cursor-pointer text-gray-400 hover:text-red-500" onClick={() => { setShowTaskModal(false); setModalText(''); }} />
              </div>
              <input 
                autoFocus 
              ...
              <div className="flex gap-4">
                 <button onClick={() => { setShowTaskModal(false); setModalText(''); }} className="flex-1 py-4 text-gray-400 font-bold uppercase text-[10px] tracking-widest hover:text-gray-600 transition-all">Cancelar</button>
                 <button onClick={handleTaskSubmit} className="flex-1 py-4 bg-[#333333] text-white rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-gold transition-all shadow-lg">Gravar na Lista</button>
              </div>
           </div>
        </div>
      )}
